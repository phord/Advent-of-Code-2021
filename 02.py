...
def parse(input):
    return [x.split() for x in input.splitlines() if x]

def part1(cmds):
    depth = sum([ int(y) for (x,y) in cmds if x == "down"]) - sum([ int(y) for (x,y) in cmds if x == "up"])
    pos = sum([ int(y) for (x,y) in cmds if x == "forward"])
    print(f"Depth: {depth}  Pos: {pos}")
    return depth * pos
